...
# Função auxiliar para verificar o Socket Interno (Camada 4)
# Exemplo de chamada: port=$(extract_port_simple "http://127.0.0.1:8200")
extract_url_port() {
    local url="${1}"
    
    # 1. Remove tudo até o último ":"
    # 2. Remove qualquer barra "/" que venha depois (ex: :8200/config -> 8200)
    local port="${url##*:}"
    port="${port%%/*}"

    # Validação: Se o resultado não for um número, a URL não tinha porta explícita
    if [[ "$port" =~ ^[0-9]+$ ]]; then
        echo "$port"
    else
        # Retorno padrão baseado no protocolo se a porta for omissa
        if [[ "$url" =~ ^https:// ]]; then
            echo "443"
        else
            echo "80"
...
require_containers_ready() {
    ...
    for _var_name_ in "$@"; do
        ...
        # 1. Verifica se o container existe e está rodando
        local status=$(docker inspect -f "{{.State.Running}}" "$container_name" 2>/dev/null)
        
        if [[ "$status" != "true" ]]; then
            echo "❌ [STOPPED/MISSING] $container_name" >&2
            failed_list+=("$container_name")
            continue
        fi

        # 2. Tenta validar o serviço (Wait Loop)
        # require_service_ready agora chama internamente a resolução de IP/Porta
        if ! require_service_ready "$container_name"; then
            failed_list+=("$container_name")
        fi
    done

    if [[ ${#failed_list[@]} -gt 0 ]]; then
        echo -e "\n🛑 Critical: ${#failed_list[@]} service(s) failed health check." >&2
        return 1
    fi
    return 0
}
export -f require_containers_ready
...
core_set__domain_name() {
    patch__etc_hosts() {
        ...
        require_files hosts_file || return 1
        require_vars ip_v4 ip_v6 DOMAIN

        if [ -z "$domain" ]; then
            >&2 echo "❌ Error: Domain not provided to patch__etc_hosts"
            return 1
        fi

        # 1. Copia o arquivo atual removendo linhas antigas que contenham o domínio
        # Isso garante que se o IP mudar, a linha antiga é substituída e não duplicada.
        grep -v "$domain" "$hosts_file" > "$tmp_hosts"

        # 2. Adiciona as novas entradas no final do arquivo temporário
        echo -e "\n# --- Autogenerated by core_set__domain_name ---" >> "$tmp_hosts"
        
        if [ -n "$ip_v4" ]; then
            echo -e "${ip_v4}\t${domain}" >> "$tmp_hosts"
        fi

        if [ -n "$ip_v6" ]; then
            echo -e "${ip_v6}\t${domain}" >> "$tmp_hosts"
        fi
        echo -e "# ----------------------------------------------" >> "$tmp_hosts"

        # 3. Substitui o arquivo original com segurança
        # Usamos cat + redirecionamento para manter as permissões originais do /etc/hosts    
        cat $tmp_hosts ## cat file content to decide enter sudo creds
        sudo cp "$hosts_file" "${hosts_file}.bak" # Backup de segurança
        sudo cat "$tmp_hosts" | sudo tee "$hosts_file" > /dev/null

        rm "$tmp_hosts"
        echo "✅ /etc/hosts patched for $domain"
    }
    patch__etc_hosts || return 1
}
core_get__domain_name() {
    local domain="${DOMAIN}"
    local hosts_file="/etc/hosts"
    
    # 1. Pré-requisito: A variável DOMAIN deve existir no ambiente
    if [[ -z "$domain" ]]; then
        >&2 echo "❌ [FAIL] DOMAIN variable is not set in environment."
        return 1
    fi
    ...
    # 3. Cross-Check: Verificação do Hostname do Sistema
    local current_hostname=$(hostname -f)
    if [[ "$current_hostname" != "$domain" ]]; then
        >&2 echo "⚠️  [MISMATCH] System hostname ($current_hostname) does not match \$DOMAIN ($domain)."
        return 1
    fi

    # 4. Cross-Check: Verificação do /etc/hosts
    # Procuramos uma linha que contenha exatamente o IP atual e o domínio
    local hosts_check_v4=$(grep -E "^${actual_ipv4}\s+.*${domain}" "$hosts_file")
    
    if [[ -z "$hosts_check_v4" ]]; then
        >&2 echo "❌ [FAIL] /etc/hosts is missing or has wrong IP for $domain (Expected $actual_ipv4)."
        return 1
    fi

    # 5. Verificação Opcional de IPv6 (se o IPv6 for detectado, ele deve estar no hosts)
    if [[ -n "$actual_ipv6" ]]; then
        local hosts_check_v6=$(grep -E "^${actual_ipv6}\s+.*${domain}" "$hosts_file")
        if [[ -z "$hosts_check_v6" ]]; then
            >&2 echo "⚠️  [WARNING] IPv6 $actual_ipv6 detected but not found in /etc/hosts for $domain."
            # Opcional: retornar 1 aqui se o IPv6 for obrigatório no seu setup
        fi
    fi

    # 6. Verificação de Resolução via Sistema (getent)
    # Em vez de um grep rígido, verificamos se o IP detectado ESTÁ na lista de respostas do getent
    local resolved_ips=$(getent hosts "$domain" | awk '{print $1}')
    
    local found_match=false
    for rip in $resolved_ips; do
        if [[ "$rip" == "$actual_ipv4" ]] || [[ "$rip" == "$actual_ipv6" ]]; then
            found_match=true
            break
        fi
    done
    ...
    if [ "$found_match" = false ]; then
        >&2 echo "❌ [FAIL] OS Resolver (getent) for $domain does not include $actual_ipv4."
        return 1
    fi

    echo "✅ [SUCCESS] Domain $domain is correctly mapped to $actual_ipv4"
    return 0
}
...
docker_container_name_ip() {
    ...
    # 1. Verifica se está em modo HOST
    local network_mode=$(docker inspect -f '{{.HostConfig.NetworkMode}}' "$container_name" 2>/dev/null)
    if [[ "$network_mode" == "host" ]]; then
        local resolved_ip
        # nslookup costuma cuspir várias linhas, pegamos apenas o endereço final
        resolved_ip=$(nslookup "$container_name.$network_name" 2>/dev/null | grep -A 1 "Name:" | grep "Address:" | awk '{print $2}' | head -n1)
    ...
    # 1. Tenta buscar na rede específica (sua lógica original)
    _ip=$(docker inspect -f "{{with index .NetworkSettings.Networks \"$network_name\"}}{{.IPAddress}}{{end}}" "${container_name}" 2>/dev/null)

    # 2. DISCOVER: Se falhar, busca em QUALQUER rede conectada
    if [[ -z "$_ip" || ! $_ip =~ $ipv4_regex ]]; then
        # Pega o primeiro IP de qualquer rede que não seja vazia
...
docker_container_name_ports() {
    local name="$1"
    # 3. Filtra apenas o que termina em /tcp
    # 4. Remove o sufixo /tcp e limpa duplicatas
    docker inspect -f '{{range $p, $conf := .NetworkSettings.Ports}}{{$p}} {{end}} {{range $p, $conf := .Config.ExposedPorts}}{{$p}} {{end}}' "$name" 2>/dev/null \
        | tr ' ' '\n' \
        | grep '/tcp' \
        | sed 's/\/tcp//g' \
        | sort -u \
...
core__container_name_url_candidates() {
    local name="${1:-pihole}"
    local ip=$(docker_container_name_ip "$name")
    
    # 1. Coleta portas ignorando UDP (curl não testa UDP nativamente assim)
    # 1. Extração Universal (Funciona em qualquer versão do Docker)
    # Pegamos todas as portas (NetworkSettings + Config) e filtramos apenas as TCP via grep
    local p_docker=$(docker inspect -f '{{range $p, $conf := .NetworkSettings.Ports}}{{$p}} {{end}} {{range $p, $conf := .Config.ExposedPorts}}{{$p}} {{end}}' "$name" 2>/dev/null \
        | tr ' ' '\n' \
        | grep '/tcp' \
        | sed 's/\/tcp//g' \
        | sort -u \
    ...
    # 2. Fallback para versões antigas de Docker que não aceitam 'contains' no template
    if [[ $? -ne 0 ]]; then
        p_docker=$(docker inspect -f '{{range $p, $conf := .NetworkSettings.Ports}}{{$p}} {{end}}' "$name" | grep -oP '\d+(?=/tcp)' | xargs)
    fi

    local all_ports=$(echo "$p_docker" | xargs -n1 | sort -u | xargs)

    for p in $all_ports; do
        # --- FILTRO DE PROTOCOLOS DE INFRA (Ignorar no teste de URL) ---
        if [[ "$p" == "53" || "$p" == "67" || "$p" == "123" || "$p" == "5432" || "$p" == "6379" ]]; then
            # Opcional: echo "DEBUG: Skipping infra port $p" >&2
            continue
        fi

        # --- MAPEAMENTO DE WELL-KNOWN WEB PORTS ---
        if [[ "$p" == "443" || "$p" == "8443" || "$p" == "9443" ]]; then
            echo "https://$ip:$p"
        elif [[ "$p" == "80" || "$p" == "1080" || "$p" == "8080" || "$p" == "9000" ]]; then
            echo "http://$ip:$p"
        elif [[ "$p" == "9200" ]]; then
            echo "https://$ip:$p"
            echo "http://$ip:$p"
        else
            # PORTA DESCONHECIDA (Pode ser uma Web UI customizada)
            echo "http://$ip:$p"
            echo "https://$ip:$p"
        fi
    done
...
core_http_url_status() {
    ...
    local http_status_proxy_err="^(502|503|504|404|401)$" # Adicionei 401 para o caso do oCIS
    ...
    if [[ "$input_url" =~ ^https?:// ]]; then
        urls_to_test+=("$input_url")
    else
        # Se não tem protocolo, tenta HTTPS primeiro (padrão oCIS), depois HTTP
        urls_to_test+=("https://$input_url" "http://$input_url")
    fi

    for url in "${urls_to_test[@]}"; do
        local response
        # -I (Head), -s (Silent), -L (Follow Redirects), -k (Insecure), -m (Timeout)
        response=$(curl -I -s -L -k -m 5 "$url" -w "$write_out_format" -o /dev/null)
        local curl_exit_status=$?

        # Se o curl falhou totalmente (DNS/Conexão Recusada), tenta a próxima URL da lista
        if [ $curl_exit_status -ne 0 ]; then
            continue
        fi
        ...
        # Se cair num padrão de erro de proxy ou for 000 (servidor não respondeu nada)
