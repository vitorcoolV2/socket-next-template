...
echo "🚀 Generating TLS certificates for ${HOST_NAME}..."
...
docker exec -t step-ca step ca certificate \
    ...
    --san="whoami.$HOST_NAME" \
    --san="navbar.$HOST_NAME" \
    --san="kuma.$HOST_NAME" \
    --san="portainer.$HOST_NAME" \
    --not-after=24h \
    --force
...
COMPOSE_SERVICE_PATH=$(realpath ${PWD}/../traefik/traefik)
echo "the service: $COMPOSE_SERVICE_PATH"
sudo mkdir -p $COMPOSE_SERVICE_PATH/certs
SERVICE_CERT_PATH=$(realpath "${PWD}/../traefik/traefik/certs")
...
docker cp step-ca:/home/step/web-certs/$HOST_NAME/traefik.cert.pem "${SERVICE_CERT_PATH}/traefik.cert.pem"
docker cp step-ca:/home/step/web-certs/$HOST_NAME/traefik.key.pem "${SERVICE_CERT_PATH}/traefik.key.pem"
docker cp step-ca:/home/step/certs/intermediate_ca.crt "${SERVICE_CERT_PATH}/intermediate-ca.cert.pem"
docker cp step-ca:/home/step/certs/root_ca.crt "${SERVICE_CERT_PATH}/root_ca.crt"

# 🔥 FIX: Rename files to the format Traefik expects
...
CERT_HASH=$(openssl x509 -in ${SERVICE_CERT_PATH}/traefik.cert.pem -noout -pubkey 2>/dev/null | openssl md5)
KEY_HASH=$(openssl pkey -in ${SERVICE_CERT_PATH}/traefik.key.pem -pubout 2>/dev/null | openssl md5)
...
openssl x509 -in ${SERVICE_CERT_PATH}/traefik.cert.pem -text -noout 

echo "🎉 Process completed!"
echo "📁 Certificates in: ${SERVICE_CERT_PATH}"

# Fix permissions on host
sudo chown -R root:root ${SERVICE_CERT_PATH}
sudo chmod 644 ${SERVICE_CERT_PATH}/traefik.cert.pem
sudo chmod 600 ${SERVICE_CERT_PATH}/traefik.key.pem