    ...
    while (i < args.length) {
      ...
      if (arg === '-p' || arg === '--password') {
        ...
        if (i + 1 < args.length) {
          ...
          console.error(
            '❌ Full name value is required after --full-name flag'
          );
          process.exit(1);
        }
...
/**
 * Create necessary LDAP structure
 */
function createLDAPStructure(ldapConfig) {
  try {
    console.log('🔧 Checking/Creating LDAP structure...');

    // Check if ou=users exists
    try {
      execSync(
        `ldapsearch -x -H ${ldapConfig.url} -D "${ldapConfig.bindDN}" -w "${ldapConfig.bindPassword}" -b "ou=users,${ldapConfig.baseDN}" -s base dn 2>/dev/null`,
        { encoding: 'utf8' }
      );
      console.log('✅ ou=users exists');
    } catch (error) {
      // Create ou=users
      console.log('📁 Creating ou=users...');
      const usersOU = `dn: ou=users,${ldapConfig.baseDN}
objectClass: organizationalUnit
ou: users`;
      execSync(
        `echo '${usersOU}' | ldapadd -x -H ${ldapConfig.url} -D "${ldapConfig.bindDN}" -w "${ldapConfig.bindPassword}"`,
        { encoding: 'utf8' }
      );
      console.log('✅ Created ou=users');
    }

    // Check if ou=groups exists
    try {
      execSync(
        `ldapsearch -x -H ${ldapConfig.url} -D "${ldapConfig.bindDN}" -w "${ldapConfig.bindPassword}" -b "ou=groups,${ldapConfig.baseDN}" -s base dn 2>/dev/null`,
        { encoding: 'utf8' }
      );
      console.log('✅ ou=groups exists');
    } catch (error) {
      // Create ou=groups
      console.log('📁 Creating ou=groups...');
      const groupsOU = `dn: ou=groups,${ldapConfig.baseDN}
objectClass: organizationalUnit
ou: groups`;
      execSync(
        `echo '${groupsOU}' | ldapadd -x -H ${ldapConfig.url} -D "${ldapConfig.bindDN}" -w "${ldapConfig.bindPassword}"`,
        { encoding: 'utf8' }
      );
      console.log('✅ Created ou=groups');
    }

    return true;
  } catch (error) {
    console.error('❌ Failed to create LDAP structure:', error.message);
    return false;
  }
}
...
userPassword: ${password}`;
  ...
  try {
    ...
    execSync(
      `echo '${passwordContent}' | ldapmodify -x -H ${ldapConfig.url} -D "${ldapConfig.bindDN}" -w "${ldapConfig.bindPassword}"`,
...
function deleteUser(username, ldapConfig, force = false) {
  ...
  try {
    ...
    execSync(
      `ldapdelete -x -H ${ldapConfig.url} -D "${ldapConfig.bindDN}" -w "${ldapConfig.bindPassword}" "cn=${escapedUsername},ou=users,${ldapConfig.baseDN}"`,
      { encoding: 'utf8', stdio: 'inherit' }
    );
...
function isUserInGroup(username, group, ldapConfig) {
  ...
  try {
    ...
    return result.includes(
      `dn: cn=${escapedGroup},ou=groups,${ldapConfig.baseDN}`
    );
  } catch (error) {
    return false;
  }
...
function addUserToGroup(username, group, ldapConfig) {
  ...
  try {
    const checkGroupCmd = `ldapsearch -x -H ${ldapConfig.url} -D "${ldapConfig.bindDN}" -w "${ldapConfig.bindPassword}" -b "cn=${escapedGroup},ou=groups,${ldapConfig.baseDN}" -s base dn > /dev/null 2>&1`;
    execSync(checkGroupCmd);
    groupExists = true; // Command succeeded, group exists
  } catch (error) {
    groupExists = false; // Command failed, group does not exist
  }
...
member: cn=${escapedUsername},ou=users,${ldapConfig.baseDN}`;
    ...
    try {
      execSync(
        `echo '${createGroupContent}' | ldapadd -x -H ${ldapConfig.url} -D "${ldapConfig.bindDN}" -w "${ldapConfig.bindPassword}"`,
        { encoding: 'utf8', stdio: 'inherit' }
      );
...
member: cn=${escapedUsername},ou=users,${ldapConfig.baseDN}`;
  ...
  try {
    ...
    execSync(
      `echo '${groupContent}' | ldapmodify -x -H ${ldapConfig.url} -D "${ldapConfig.bindDN}" -w "${ldapConfig.bindPassword}"`,
      { encoding: 'utf8', stdio: 'inherit' }
    );
...
async function main() {
  ...
  switch (args.command) {
    ...
    case 'create':
      ...
      if (!args.password) {
        console.error(
          '❌ Password is required for create command (use -p or --password)'
        );
        process.exit(1);
      }
      ...
      // Create LDAP structure first
      if (!createLDAPStructure(ldapConfig)) {
        console.error('❌ Cannot proceed without LDAP structure');
        process.exit(1);
      }
...
mail: ${args.username}@home2500.local`;
          ...
          execSync(
            `echo '${userContent}' | ldapadd -x -H ${ldapConfig.url} -D "${ldapConfig.bindDN}" -w "${ldapConfig.bindPassword}"`,
            { encoding: 'utf8', stdio: 'inherit' }
          );
      ...
      console.log(
        `📝 Status: ${userAlreadyExists ? 'Updated existing user' : 'Created new user'}`
      );
      break;
